...
프론트엔드 모델은 `apps/desktop/src/entities/agent-run/model/types.ts`에서
...
UI는 `apps/desktop/src/features/agent-run/ui/agent-run-panel.tsx`에 있다. 이
...
Tauri command layer는 `apps/desktop/src-tauri/src/inbound/tauri_commands.rs`에서
요청을 받는다.
...
domain model은 `apps/desktop/src-tauri/src/domain/run.rs`에 정의되어 있다. 백엔드
안전 제한은 다음과 같다.
...
ACP 구현은 `apps/desktop/src-tauri/src/infrastructure/acp/runner.rs`에 있다.
