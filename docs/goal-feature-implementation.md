...
프론트엔드 모델은 `apps/desktop/src/entities/agent-run/model/types.ts`에 정의되어
있다.
...
프론트엔드 API adapter는
`apps/desktop/src/entities/agent-run/api/goal-repository.ts`이다. 이 adapter는
...
React Query key는 `apps/desktop/src/entities/agent-run/api/query-keys.ts`에
모여 있다. goal cache key는 `["goal", workingDirectory]`이므로 각 worktree가
독립적인 cached goal을 가진다.

## Agent Run Panel 통합

goal UI는 `apps/desktop/src/features/agent-run/ui/agent-run-panel.tsx`에
구현되어 있다.
...
command는 `apps/desktop/src-tauri/src/lib.rs`에 등록되어 있고,
`apps/desktop/src-tauri/src/inbound/tauri_commands.rs`에 구현되어 있다.
...
백엔드 domain type은 `apps/desktop/src-tauri/src/domain/goal.rs`에 있다.

repository port는 `apps/desktop/src-tauri/src/domain/goal_repository.rs`이다.

- `load_goals() -> Vec<ThreadGoal>`
- `save_goals(goals)`

application service는 `apps/desktop/src-tauri/src/application/goal_service.rs`이다.
...
outbound adapter는
`apps/desktop/src-tauri/src/infrastructure/json_goal_repository.rs`이다.
...
`apps/desktop/src-tauri/src/application/goal_service.rs`에는 다음 동작에 대한
