...
/**
 * commit DAG를 lane/connection 기반 graph row로 변환한다.
 * first-parent 체인을 mainline(lane 0)으로 추적하고, merge 시 새 lane을 할당한다.
 * git-explorer와 agentic-workbench가 공유하는 순수 함수(DOM 비의존).
 */
export function computeGitGraphRows(commits: GitGraphCommit[]) {
  ...
  commits.forEach((commit, index) => {
    ...
    commit.parents.forEach((parentHash, parentPosition) => {
      ...
      const color =
        parentPosition === 0 ? row.color : (commitColors.get(parentHash) ?? colorFor(parentHash));
      const type =
        row.lane === parentLane
          ? "vertical-bottom"
          : parentPosition > 0
